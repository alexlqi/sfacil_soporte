...
if($search)
    $showing="Resultados de busqueda: $showing";
...
<form action="tickets.php" method="get" id="ticketSearchForm">
    ...
    <select name="status">
        <option value="">&mdash; Cualquier Estado &mdash;</option>
        <option value="open"
            <?php echo ($status=='open')?'selected="selected"':'';?>>Abierto (<?php echo $thisclient->getNumOpenTickets(); ?>)</option>
        <?php
        if($thisclient->getNumClosedTickets()) {
            ?>
        <option value="closed"
            <?php echo ($status=='closed')?'selected="selected"':'';?>>Cerrado (<?php echo $thisclient->getNumClosedTickets(); ?>)</option>
        <?php
        } ?>
    </select>
    <input type="submit" value="Ir">
</form>
<a class="refresh" href="<?php echo Format::htmlchars($_SERVER['REQUEST_URI']); ?>">Actualizar</a>
<table id="ticketTable" width="800" border="0" cellspacing="0" cellpadding="0">
    ...
    <thead>
        <tr>
            ...
            <th width="100">
                <a href="tickets.php?sort=date&order=<?php echo $negorder; ?><?php echo $qstr; ?>" title="Sort By Date">Fecha de Creación</a>
            </th>
            <th width="80">
                <a href="tickets.php?sort=status&order=<?php echo $negorder; ?><?php echo $qstr; ?>" title="Sort By Status">Estado</a>
            </th>
            <th width="300">
                <a href="tickets.php?sort=subj&order=<?php echo $negorder; ?><?php echo $qstr; ?>" title="Sort By Subject">Asunto</a>
            </th>
            <th width="150">
                <a href="tickets.php?sort=dept&order=<?php echo $negorder; ?><?php echo $qstr; ?>" title="Sort By Department">Departamento</a>
            </th>
...
<?php
if($res && $num>0) {
    echo '<div>&nbsp;Pag.:'.$pageNav->getPageLinks().'&nbsp;</div>';
}
?>
