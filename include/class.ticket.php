...
class Ticket {
    ...
    function isOpen() {
        return (strcasecmp($this->getStatus(),'Abierto')==0);
    }
    ...
    function isClosed() {
        return (strcasecmp($this->getStatus(),'Cerrado')==0);
    ...
    function onAssign($assignee, $comments, $alert=true) {
        ...
        $comments = $comments?$comments:'Ticket asignado';
   ...
   /*============== Static functions. Use Ticket::function(params); =============nolint*/
    ...
    static function open($vars, &$errors) {
        ...
        if($vars['source'] && !in_array(strtolower($vars['source']),array('email','phone','chat','inbox','other' )))
