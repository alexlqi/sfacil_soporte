...
//          'LEFT JOIN '.FORM_FIELD_TABLE.' field ON field.id=17 '.
//          'WHERE field.name = "%1$s" AND entry.object_type="T")';

//$dynfields='(SELECT GROUP_CONCAT(fev.value SEPARATOR ">" ) FROM OST_FORM_ENTRY_VALUES fev WHERE fev.field_id>= 42 AND  
//    fev.entry_id IN (SELECT fe.id FROM OST_FORM_ENTRY fe WHERE fe.object_id = ticket.ticket_id) 
//)';
        ...
        <tr>
	        ...
	        <th width="70">
                <a  <?php echo $date_sort; ?> href="tickets.php?sort=date&order=<?php echo $negorder; ?><?php echo $qstr; ?>"
                    title="Sort By Date <?php echo $negorder; ?>">Fecha</a></th>
	        <th width="280">
