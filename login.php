...
if ($_POST && isset($_POST['luser'])) {
    if (!$_POST['luser'])
        $errors['err'] = 'Nombre de Usuario o Email válidos es requerido';
...
elseif ($_POST && isset($_POST['lticket'])) {
    ...
    elseif (($user = UserAuthenticationBackend::process($_POST['lemail'],
        ...
        $user->sendAccessLink();
        $msg = sprintf("%s - enlace de acceso enviado a tu email!",
            $user->getName()->getFirst());
        $_POST = null;
    } elseif(!$errors['err']) {
        $errors['err'] = 'Email o número de ticket invalido - inténtalo nuevamente!';
    }
}
