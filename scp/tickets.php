            ...
            switch(strtolower($_POST['do'])):
                ...
                case 'reopen':
                    ...
                    if(!$thisstaff->canCloseTickets() && !$thisstaff->canCreateTickets()) {
                        ...
                        $errors['err'] = 'Ticket ha sido abierto!';
...
if($cfg->showAnsweredTickets()) {
    $nav->addSubMenu(array('desc'=>'Abierto ('.number_format($stats['open']+$stats['answered']).')',
                            'title'=>'Abrir Tickets',
                            'href'=>'tickets.php',
                            'iconclass'=>'Ticket'),
    ...
    if($stats) {
        $nav->addSubMenu(array('desc'=>'Abierto ('.number_format($stats['open']).')',
                               'title'=>'Open Tickets',
                               'href'=>'tickets.php',
                               'iconclass'=>'Ticket'),
                            (!$_REQUEST['status'] || $_REQUEST['status']=='open'));
    ...
    if($stats['answered']) {
        $nav->addSubMenu(array('desc'=>'Contestado ('.number_format($stats['answered']).')',
...
if($stats['assigned']) {
    ...
    $nav->addSubMenu(array('desc'=>'Mis Tickets ('.number_format($stats['assigned']).')',
...
if($thisstaff->showAssignedOnly() && $stats['closed']) {
    $nav->addSubMenu(array('desc'=>'Mis Tickets Cerrados ('.number_format($stats['closed']).')',
    ...
    $nav->addSubMenu(array('desc'=>'Tickets Cerrados ('.number_format($stats['closed']).')',
...
if($thisstaff->canCreateTickets()) {
    $nav->addSubMenu(array('desc'=>'Nuevo Ticket',
...
$ost->addExtraHeader('<script type="text/javascript" src="js/ticket.js?4827655"></script>');
